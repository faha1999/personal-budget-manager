...
export function ExpenseForm({
  ...
  const onSubmit = handleSubmit(async (values) => {
    ...
    const res = await fetch(isEdit ? `/api/expenses/${initialValues!.id}` : "/api/expenses", {
    ...
    if (!res.ok) {
      ...
      alert(j?.error || `Failed to ${isEdit ? "update" : "create"} expense.`);
      return;
    }
  ...
  return (
    <form onSubmit={onSubmit} className="card p-5 space-y-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          <div className="text-sm font-semibold">{isEdit ? "Edit Expense" : "Add Expense"}</div>
