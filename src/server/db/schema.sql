-- TODO: Define base schema for users, categories, accounts, transactions, investments, goals, loans, and related history tables.

-- Schema snapshot (managed by migrations in ./migrations)
...
CREATE TABLE IF NOT EXISTS accounts (
  ...
  name                  TEXT NOT NULL,
  bank_name             TEXT,
  type                  TEXT NOT NULL, -- BANK | CASH | MOBILE_WALLET | OTHER
...
CREATE TABLE IF NOT EXISTS transactions (
  ...
  note            TEXT,
  occurred_at     TEXT NOT NULL, -- ISO string
  created_at      TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now')),
  updated_at      TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now')),
  FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS investments (
  ...
  user_id         TEXT NOT NULL,
  name            TEXT NOT NULL,
  type            TEXT NOT NULL, -- STOCK | FUND | DPS | FDR | CRYPTO | GOLD | OTHER
  provider        TEXT,
  currency        TEXT NOT NULL DEFAULT 'BDT',
  units           REAL,
  note            TEXT,
